...
2a. 仅当用户明确要求 bounded semantic query / Region Query Reader 性能验收时，先立即记录本轮 smoke_start，再改读 references/reader-bounded-query-contract.md，并执行该文件定义的独立 smoke；必须停在 reader-partial-observations.json，不得继续 ReaderCapture / linker / Resolver / Gate A / Planner / Runner / NX。每个 query 只能由 Agent 直接视觉读取 query.image_path 一次，严禁用 PIL/Pillow、OpenCV、System.Drawing、PowerShell/.NET 图像代码、GetPixel/LockBits、ASCII 图、OCR、像素坐标测量、边线检测或任何二次程序化图像分析；证据不足直接 structured unresolved。visual semantic 计时必须持续到当前轮唯一 reader-semantic-answers.json 完整写出，不能在最后一张 crop 读取完成时提前结束；smoke 停止时必须记录 smoke_end 并汇报真实 wall clock。该 smoke 未经验收前不得替代正常 Mode B 主线。
