    ...
    def _shape(self) -> "ObservationDimensionEndpoint":
        if self.role == "entity_center":
            ...
            if self.candidate_entity_keys:
                raise ValueError(
                    "entity_center endpoint must not carry candidate_entity_keys"
                )
            ...
            if self.basis is None:
                raise ValueError(
                    "entity_center endpoint requires centerline/center_mark/"
                    "explicit_midline basis"
                )
            ...
            if self.candidate_entity_keys:
                raise ValueError(
                    f"{self.role} endpoint must not carry candidate_entity_keys"
                )
            ...
            if (
                self.unresolved_kind == "ambiguous_owner"
                and not self.candidate_entity_keys
            ):
                raise ValueError(
                    "ambiguous_owner endpoint requires candidate_entity_keys"
                )
            if (
                self.unresolved_kind != "ambiguous_owner"
                and self.candidate_entity_keys
            ):
                raise ValueError(
                    f"{self.unresolved_kind} endpoint must not carry "
    ...
    def _semantic_shape(self) -> "ObservationUnresolved":
        if self.kind == "feature_value":
            if len(self.entity_keys) != 1:
                raise ValueError(
                    "feature_value unresolved requires exactly one entity_key"
                )
            if not self.field:
                raise ValueError(
    ...
    def _keys_and_references(self) -> "ReaderObservations":
        ...
        for entity in self.entities:
            if entity.view_key not in view_set:
                raise ValueError(
                    f"entity {entity.key!r} references unknown view "
                    f"{entity.view_key!r}"
                )

        for association in self.associations:
            missing = [
                item for item in association.entity_keys if item not in entity_set
            ]
            if missing:
                raise ValueError(
                    f"association references unknown entity keys {missing}"
                )

        for value in self.values:
            if value.entity_key not in entity_set:
                raise ValueError(
                    f"value references unknown entity key {value.entity_key!r}"
                )
        ...
        for alignment in self.datum_alignments:
            if alignment.entity_key not in entity_set:
                raise ValueError(
                    "datum alignment references unknown entity key "
        ...
        for unresolved in self.unresolved:
            ...
            if missing:
                raise ValueError(
                    f"unresolved references unknown entity keys {missing}"
                )
            if (
                ...
                raise ValueError(
                    "unresolved references unknown dimension key "
...
def assemble_reader_capture(observations: ReaderObservations) -> ReaderCapture:
    ...
    view_ids = {
        item.key: f"V{index:03d}"
        for index, item in enumerate(observations.views, start=1)
    }
    entity_ids = {
        item.key: f"E{index:03d}"
        for index, item in enumerate(observations.entities, start=1)
    }
    dimension_ids = {
    ...
    if contract_errors:
        raise ReaderObservationAssemblyError(
            "ReaderCapture contract validation failed: "
