...
2a. 仅当用户明确要求 bounded semantic query / Region Query Reader 性能验收时，改读 references/reader-bounded-query-contract.md，并执行该文件定义的独立 smoke；必须停在 reader-partial-observations.json，不得继续 ReaderCapture / linker / Resolver / Gate A / Planner / Runner / NX。该 smoke 未经验收前不得替代正常 Mode B 主线。
