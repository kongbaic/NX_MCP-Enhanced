...
- **禁止 OCR 主导读图**：不得用 Python/PIL/OpenCV 做全图分块、连续裁剪、放大、
  锐化/二值化或多轮 OCR。
- 整图读取后默认 **0 次**局部复核；仅当“恰好 1 个关键标注”是唯一阻塞项时，
  最多允许 **1 次**定点复核，且只能看该区域一次。
- 清晰厂商图存在规格表/参数表时，表格是尺寸数值主源；视图用于解释几何含义和查冲突，
  禁止把同一组表格数值逐个放大尺寸线重复确认。
- 超过预算仍无法确定 → 立即 `unresolved`，结束 Gate A；不得继续追图。
