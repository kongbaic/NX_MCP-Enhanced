...
- projection alignment、shared centerline / center mark、同心圆只说明 correspondence / coaxiality 候选；same physical identity 的锁定必须按 `drawing-reader.md` 的 identity-specific evidence 规则执行。
- `M-series thread / through hole / counterbore / countersink` 可在不同视图表达同一 feature 或 coaxial group；共享中心线不能区分这两种情况。邻近、相同数值或相同 axis 本身不是 association evidence。
- matching specification 必须是可追踪到同一物理项的明确 specification；互补但不同的加工语义不能仅凭共轴当作 matching specification。
- same-feature association 只识别 identity；不同 feature 的 alignment/connected/spacing 仍须建立 evidence-backed relation。
