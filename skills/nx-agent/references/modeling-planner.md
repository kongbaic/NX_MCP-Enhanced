...
当输入缺少会改变最终几何的尺寸时，Planner 可以向用户列出推荐参数帮助确认，但在
展示前必须先做确定性的几何闭合检查：

- 方位描述与坐标范围必须一致；“位于 X/Y 某一侧”不能与给出的实体范围矛盾。
- 推荐孔位、PCD、板厚、边距、圆角/倒角不得互相穿插。若圆孔半径 `r` 邻近
  半径 `R` 的圆角边，且需要保留完整孔口，则孔心到该边的距离要求
  `d >= r + R`。
- 若检查失败，禁止把这组值作为推荐；只允许继续询问，或调整尚未被用户指定的参数。
- 用户已明确给出的相互干涉尺寸不得自动修正，只能披露冲突并等待确认。
...
- 输入本身明确提供几何等价且可执行的 `surrogate_geometry` 时优先按输入执行。否则只能消费 `validate-drawing` 的机器 resolver 结果：通用 metric parser 解析 nominal diameter/pitch，bare designation 由 project-supported coarse-pitch subset 提供 pitch，tap-drill surrogate 统一按 `nominal_diameter - pitch` 计算。Planner 禁止维护 thread-size → 最终孔径表，也禁止自行计算或改写 surrogate diameter；不得省略 start_offset 等字段以依赖 Loader 默认。
