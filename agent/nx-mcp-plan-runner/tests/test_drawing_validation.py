...
class DrawingSchemaNormalizationTests(unittest.TestCase):
    ...
    @staticmethod
    def run_submit(
        draft: Path, output: Path, payload: bytes
    ) -> tuple[int, dict]:
        args = type("Args", (), {"draft": str(draft), "out": str(output)})()
        stdout = io.StringIO()
        stdin = io.TextIOWrapper(io.BytesIO(payload), encoding="utf-8")
        with mock.patch.object(R.sys, "stdin", stdin), redirect_stdout(stdout):
            return_code = R._cmd_submit_semantic_draft(args)
        return return_code, json.loads(stdout.getvalue())
    ...
    def valid_submission_bytes() -> bytes:
        return (
            json.dumps(
                DrawingSchemaNormalizationTests.gate_a_range_alias_data(),
                ensure_ascii=False,
                indent=2,
            )
            + "\n"
    ...
    def test_submit_first_valid_submission_fixes_raw_draft(self) -> None:
        payload = self.valid_submission_bytes()
        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
            result, report = self.run_submit(draft, output, payload)
            self.assertEqual(0, result)
            self.assertTrue(report["submission_accepted"])
            self.assertTrue(report["canonicalization_attempted"])
            self.assertEqual(payload, draft.read_bytes())
            self.assertTrue(output.exists())

    def test_submit_gate_failure_preserves_raw_draft_and_writes_nothing(self) -> None:
        data = self.gate_a_range_alias_data()
        data["source_ledger"] = [
            item for item in data["source_ledger"] if item["id"] != "S_SLOT_WIDTH"
        ]
    ...
    def test_submit_second_submission_is_rejected_without_overwrite(self) -> None:
        first = b"{malformed-first"
        second = self.valid_submission_bytes()
        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
            first_result, _ = self.run_submit(draft, output, first)
            second_result, report = self.run_submit(draft, output, second)
            self.assertEqual(1, first_result)
            self.assertEqual(1, second_result)
            self.assertFalse(report["submission_accepted"])
            self.assertTrue(report["already_submitted"])
            self.assertFalse(report["canonicalization_attempted"])
            self.assertEqual(first, draft.read_bytes())
            self.assertFalse(output.exists())

    def test_submit_second_submission_does_not_run_canonicalizer(self) -> None:
        payload = self.valid_submission_bytes()
        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
            with mock.patch.object(
                R, "_canonicalize_drawing", wraps=R._canonicalize_drawing
            ) as canonicalize:
                first_result, _ = self.run_submit(draft, output, payload)
                second_result, report = self.run_submit(draft, output, payload)
            self.assertEqual(0, first_result)
            self.assertEqual(1, second_result)
            self.assertEqual(1, canonicalize.call_count)
            self.assertTrue(report["already_submitted"])

    def test_submit_second_submission_does_not_read_stdin(self) -> None:
        class RejectRead:
            @property
            def buffer(self):
                return self
            ...
            def read(self):
                raise AssertionError("second submission stdin must not be consumed")

        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
            original = b"first-submission"
            draft.write_bytes(original)
            args = type("Args", (), {"draft": str(draft), "out": str(output)})()
            stdout = io.StringIO()
            with mock.patch.object(R.sys, "stdin", RejectRead()), redirect_stdout(stdout):
                result = R._cmd_submit_semantic_draft(args)
            report = json.loads(stdout.getvalue())
            self.assertEqual(1, result)
            self.assertTrue(report["already_submitted"])
            self.assertEqual(original, draft.read_bytes())

    def test_submit_first_malformed_json_is_saved(self) -> None:
        payload = b'{"broken":'
        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
    ...
    def test_submit_malformed_first_closes_valid_second_submission(self) -> None:
        malformed = b"not-json"
        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
            self.run_submit(draft, output, malformed)
            result, report = self.run_submit(
                draft, output, self.valid_submission_bytes()
            )
            self.assertEqual(1, result)
            self.assertTrue(report["already_submitted"])
            self.assertEqual(malformed, draft.read_bytes())
            self.assertFalse(output.exists())

    def test_submit_normalization_failure_preserves_draft(self) -> None:
        payload = json.dumps({
            "features": [{"id": "F_BAD", "range_z": {"start": 1, "to": 2}}]
        }).encode("utf-8")
        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
    ...
    def test_submit_canonicalizer_exception_keeps_submission_closed(self) -> None:
        payload = self.valid_submission_bytes()
        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
            with mock.patch.object(
                R, "_canonicalize_drawing", side_effect=RuntimeError("boom")
            ):
                result, report = self.run_submit(draft, output, payload)
            self.assertEqual(1, result)
            self.assertTrue(report["submission_accepted"])
            self.assertIn(
                "canonicalizer internal error (RuntimeError): boom", report["errors"]
            )
            self.assertEqual(payload, draft.read_bytes())
            self.assertFalse(output.exists())
            retry_result, retry = self.run_submit(draft, output, payload)
            self.assertEqual(1, retry_result)
            self.assertTrue(retry["already_submitted"])

    def test_submit_concurrent_submissions_accept_exactly_one(self) -> None:
        payload = self.valid_submission_bytes()
        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
                calls = list(pool.map(lambda _: invoke(), range(2)))
            reports = [json.loads(call.stdout) for call in calls]
            self.assertEqual([0, 1], sorted(call.returncode for call in calls))
            self.assertEqual(1, sum(bool(r["submission_accepted"]) for r in reports))
            self.assertEqual(1, sum(bool(r["already_submitted"]) for r in reports))
            self.assertEqual(payload, draft.read_bytes())
            self.assertTrue(output.exists())

    def test_submit_preexisting_draft_fails_closed(self) -> None:
        original = b"first-submission"
        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
    ...
    def test_submit_preexisting_drawing_reports_workspace_not_clean(self) -> None:
        stale = b"stale-drawing"
        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
            ...
            self.assertEqual(1, result)
            self.assertTrue(report["workspace_not_clean"])
            self.assertFalse(report["submission_accepted"])
            self.assertFalse(draft.exists())
            self.assertEqual(stale, output.read_bytes())

    def test_submit_rejects_same_input_and_output_before_write(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "semantic-draft.json"
            result, report = self.run_submit(path, path, self.valid_submission_bytes())
            self.assertEqual(1, result)
            self.assertFalse(report["submission_accepted"])
            self.assertFalse(report["already_submitted"])
            self.assertFalse(path.exists())

    def test_submit_pass_writes_gate_valid_drawing_atomically(self) -> None:
        payload = self.valid_submission_bytes()
        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
    ...
    def test_submit_failure_leaves_no_temp_files(self) -> None:
        payload = b"malformed"
        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
            result, _ = self.run_submit(draft, output, payload)
            self.assertEqual(1, result)
            self.assertEqual([], list(Path(tmp).glob(".drawing-canonical-*.tmp")))

    def test_submit_pass_leaves_no_temp_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            draft = Path(tmp) / "semantic-draft.json"
            output = Path(tmp) / "drawing.json"
