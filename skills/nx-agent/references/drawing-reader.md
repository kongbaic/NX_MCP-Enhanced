...
- 第一轮整图读取后默认**不做局部复核**。只有当“恰好 1 个关键标注”是 Gate A
  唯一阻塞项、且整图中确实不可读时，才允许最多 **1 次**定点局部复核；禁止任何第二次复核。
...
3. 默认直接进入尺寸闭合；只有“恰好 1 个关键标注”是唯一阻塞项时，才允许最多 1 次
   定点局部复核。不得启动批量 OCR、全图分块、多轮裁剪或“为了确认箭头指向”不断放大。
4. 只用图中明确数值进行尺寸闭合。
5. 仍不确定的关键项直接 `unresolved`；不要继续图像处理。
6. 输出结构化 JSON。

### 2.1 清晰厂商图 Fast Gate A

对清晰、规则排版的厂商/CAD 导出工程图，优先走 Fast Gate A：

1. 一次整图读取完成：标题栏/规格表 + 主视图/俯视图/侧视图。
2. 若存在规格表/参数表，**数值以表格为主源**，视图只用于确认几何含义、方向、数量和冲突；
   禁止为了把表中每个数再次从尺寸箭头上“复读确认”而逐区放大。
3. Gate A 只输出闭合所需的最小结构：`overall_dimensions / coordinate_system /
   features / unresolved / dimension_conflicts / dimension_closure / capability_boundaries`。
   材料、表面、公差、制造备注可保留摘要，但不得为了完整抄录它们延长读图。
4. Fast Gate A PASS 后，下游必须复用本次已提取的结构化结果；**禁止为了 Planner 再读一遍原图**。
5. 若一次整图读取已经发现任何关键 unresolved/conflict，直接 BLOCKED；除“恰好 1 个不可读标注”
   这一唯一例外外，不做局部复核。

**性能门禁（Mode B / Gate A）**：
- 清晰单张工程图的 Gate A 应是一次读图任务，不是图像处理流水线。
- 禁止为了追求“100% 置信度”穷举所有区域。
- 图像辅助操作预算：整图读取之外默认 0 次；唯一例外最多 1 次定点局部复核。
