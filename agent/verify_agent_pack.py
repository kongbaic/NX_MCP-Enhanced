...
def main() -> None:
    ...
    for token in (
        ...
        "coordinate正确不能代替measured ownership",
    ...
    for token in (
        ...
        "首次输出就使用当前drawing contract",
        "必需root为`overall_dimensions / coordinate_system / features / source_ledger / derived / unresolved / dimension_conflicts / dimension_closure`",
        "measured quantity和真实endpoint ownership",
        "dimension-bearing number 必须通过 `source`",
        "路径必须落到draft中已存在的scalar leaf",
        "feature:<id>.centerline.<axis>",
        "feature:<id>.explicit_centers.<index>.<coordinate-index>",
        "不猜path/semantic",
        "canonicalizer只修无损representation",
        ...
        "direct witness锁定后",
        "先为每个view记录projected geometry与annotation endpoints",
        ...
        "blocking unresolved不得与同一字段的猜测concrete value/writer并存",
        "source entry必须有`id + semantic`",
        "旧`type`字段不是source semantic",
        "relation source不得用`target`",
        "derived entry必须是",
        "禁止string arithmetic",
        "`dimension_closure`必须是`{\"status\":\"closed|incomplete|conflict\"}`",
        "`links`必须包含前三个paths",
        "直接包含横向scalar坐标的object",
        "禁止对象容器target、抽象`.center`",
        "start side证据不足时保持缺省/`null`",
        "Closure is validation only",
    ):
