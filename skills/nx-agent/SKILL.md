...
2a. 仅当用户明确要求 bounded semantic query / Region Query Reader 性能验收时，先立即记录本轮 smoke_start。新的性能验收默认只读 references/reader-bounded-token-contract.md，执行 Bounded Semantic Token Reader v2：每个 query 只能由 Agent 直接视觉读取 query.image_path 一次，并在该 query 后立即写出一个 reader-semantic-<query_id>.json 紧凑事实文件；全部 query 完成后只允许调用 deterministic assemble-reader-semantic-regions 生成严格 reader-semantic-answers.json 与 reader-partial-observations.json。Agent 不再手写完整严格 answers schema。只有用户明确要求 legacy strict smoke 时才读 references/reader-bounded-query-contract.md。两种 smoke 都必须停在 reader-partial-observations.json，不得继续 ReaderCapture / linker / Resolver / Gate A / Planner / Runner / NX；严禁 PIL/Pillow、OpenCV、System.Drawing、PowerShell/.NET 图像代码、GetPixel/LockBits、ASCII 图、OCR、像素坐标测量、边线检测或任何二次程序化图像分析；证据不足保持 unresolved。该 smoke 未经验收前不得替代正常 Mode B 主线。
