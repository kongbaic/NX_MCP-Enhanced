# Candidate-Addressed Semantic Reader v3.1 Contract
...
chooses and visually locates the questions before the Agent sees the region.
...
Each query `image_path` must point to the deterministic candidate overlay
generated by `prepare-reader-input`. The overlay is the original region crop
with locator marks for listed `DGxx` candidates. Locator marks carry no
engineering semantics: they do not assert that a candidate is a dimension, do
not supply a numeric value, and do not decide endpoint ownership.
...
2. Use the visible `DGxx` overlay labels only to locate the listed target IDs.
   Do not treat the overlay line or witness marks as semantic truth.
3. Determine only the query's `view_kind`: `front`, `side`, or `top`.
4. Answer every listed `dimension_target` exactly once.
5. Do not search the region for additional entities, dimensions, tolerances,
   datums, notes, holes, slots, profiles, or other facts that are not represented
   by a listed target.
6. Do not invent additional target IDs.
7. Use only the anchor refs listed in that target's `witness_hints`.
8. If a listed target cannot be decided from this single read, return
   `classification="uncertain"`. Do not reopen or zoom the image.
9. Do not open the original drawing, unannotated region crop, contact sheet, bucket crops, other region
   crops, history, source, old answers, or any unlisted image.
10. Do not use PIL/Pillow, OpenCV, System.Drawing, PowerShell/.NET image code,
...
11. Immediately write exactly one answer file:
    `<workspace_root>/reader-candidate-<query_id>.json`.
12. After writing that file, do not edit, patch, diff, reread, or rename it.
