...
若为了让用户快速确认而给出“推荐参数/默认建议”，推荐值在展示给用户前必须先做
**建议参数闭合检查**，禁止把未经自检的猜测当成推荐：
- **方位文字与坐标一致**：例如声称竖板位于底板 `Y=max` 一侧且厚度为 `t`，
  若板体位于底板包络内部，则对应范围必须是 `[Ymax-t, Ymax]`，不能同时写成
  `Y=min..min+t`。
- **孔/切口与圆角倒角净距检查**：若圆孔半径为 `r`、相邻边将做半径 `R`
  的圆角，且要求该面的孔口保持完整圆形，则孔心到该名义边的距离 `d` 至少满足
  `d >= r + R`。不满足时不得把两组值同时作为“推荐参数”；应改为继续询问，
  或只调整仍未由用户指定的那个参数并明确说明。
- 同样检查推荐的孔位/PCD、板厚、边距、圆角/倒角之间是否产生几何相交。
- 若冲突来自用户已经明确指定的尺寸，禁止擅自改值；建模前应明确披露冲突并让用户决定。
