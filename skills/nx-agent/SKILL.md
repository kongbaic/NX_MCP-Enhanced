...
2a. 仅当用户明确要求 bounded semantic query / Region Query Reader 性能验收时，改读 references/reader-bounded-query-contract.md，并执行该文件定义的独立 smoke；必须停在 reader-partial-observations.json，不得继续 ReaderCapture / linker / Resolver / Gate A / Planner / Runner / NX。每个 query 只能由 Agent 直接视觉读取 query.image_path 一次，严禁用 PIL/Pillow、OpenCV、System.Drawing、PowerShell/.NET 图像代码、GetPixel/LockBits、ASCII 图、OCR、像素坐标测量、边线检测或任何二次程序化图像分析；证据不足直接 structured unresolved。该 smoke 未经验收前不得替代正常 Mode B 主线。
