...
```text
python runner.py validate-drawing <drawing.json> --new-task
# schema-only retry in the same interpretation:
python runner.py validate-drawing <drawing.json> --task-root <returned-task-root.json>
...
python runner.py build <frozen-plan.json> <out.json> [--drawing <drawing.json>] [--task-root <task.json>]
python runner.py test  #（等价：运行 tests/test_plan_resolution.py）
```

- `validate-drawing`：纯本地 Gate A 结构/证据校验；首次调用以 `--new-task` 由 Runner 生成并持久化 `mode_b_task_id`，返回的 `task_root` 是同一次解释的不可变 lineage 根。schema retry 使用该 `--task-root`，其 geometry/evidence projection 必须与首次输入等价。同一次调用先执行严格 schema-only normalization，并用 geometry-semantic projection 证明归一化前后语义一致；检查 source ownership、derived target、required geometry evidence 与 count conservation，不触 NX。normalizer 不会把 null/unresolved 补成几何值。Gate A 首次 PASS 后冻结 canonical drawing projection 与 `gate_a_geometry_sha256`。输出同时列出机器解析的 `thread_surrogates`：metric designation 被解析为 nominal diameter/pitch，bare designation 的 pitch 来自明确声明的 project-supported coarse-pitch subset，tap-drill 直径统一由 `nominal - pitch` 计算；同时输出只读 `thread_geometries` 供 Gate B 核对实际 operation，resolver 不补 placement/extent。
...
- `build`：冻结 → 可执行转换（无 NX）。Mode B 必须传 `--drawing` 和 Gate A 返回的 `--task-root`；build 只能消费该 task 已冻结的 drawing。required thread 优先使用 drawing 输入 surrogate，否则只允许消费 `validate-drawing` 返回的参数化 resolver 结果。Gate B 会重新计算 resolver，并从实际 `nx_hole` 或 sketch-circle + subtract operation 规范化 axis、center、depth、canonical axial interval 与 count；任一项不同、依赖 Loader 默认、缺失支持集 pitch 或 provenance 不符都会失败。drawing/plan 语义指纹、canonical executed geometry 与 approximation 明细写入 executable，供 repair lineage 与最终报告校验。
